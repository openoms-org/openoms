...
## ADR-026: Redis Required for Shared Runtime State Outside Development
- **Date:** 2026-05-07
- **Context:** API startup previously continued with process-local memory stores when `REDIS_URL` was invalid or Redis was unavailable. In multi-replica deployments this split refresh token rotation, logout/token revocation, login lockout, rate limits, OAuth state, WebSocket tickets, and worker locks per pod.
- **Decision:** Redis is required at API startup outside `development`. `ALLOW_IN_MEMORY_STATE=true` is the only override and is reserved for explicit single-node self-hosted deployments. Helm blocks that override when API autoscaling or multiple API/worker replicas are configured.
- **Files:** `apps/api-server/cmd/server/main.go`, `apps/api-server/internal/config/config.go`, `deploy/helm/openoms/templates/configmap.yaml`, `deploy/helm/openoms/templates/validations.yaml`
- **Consequences:** Production and staging fail closed if Redis is unreachable or misconfigured. Development keeps the ergonomic in-memory fallback. Self-hosters without Redis must opt in deliberately and stay single-node.
