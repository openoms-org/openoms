...
func NewAmazonOrderPoller(pool *pgxpool.Pool, encryptionKey []byte, orderRepo repository.OrderRepo, logger *slog.Logger) *AmazonOrderPoller {
...
func (a *AmazonOrderPoller) Name() string {
	return "amazon_order_poller"
}

func (a *AmazonOrderPoller) Interval() time.Duration {
	return 2 * time.Minute
}

func (a *AmazonOrderPoller) Run(ctx context.Context) error {
	tis, err := ListActiveIntegrations(ctx, a.pool, "amazon")
	if err != nil {
		return err
	}
	...
	for _, ti := range tis {
		...
		for _, mo := range orders {
			if err := database.WithTenant(ctx, a.pool, ti.TenantID, func(tx pgx.Tx) error {
				existing, err := a.orderRepo.FindByExternalID(ctx, tx, "amazon", mo.ExternalID)
				if err != nil {
					return err
				}
				if existing != nil {
	...
	a.logger.Info("amazon order poller completed", "tenants", len(tis), "orders", totalOrders)
	return nil
}
