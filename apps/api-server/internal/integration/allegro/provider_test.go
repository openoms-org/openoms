...
func TestFindExistingWzA_HydratesUUIDWhenCheckoutWaybillEmpty(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/order/checkout-forms/2d8367d0-9c96-11f1-bd08-9328d2ed1733/shipments":
			_, _ = w.Write([]byte(`{"shipments":[{"id":"cb92efe4-1b2f-4cac-9e35-da69b82b9482","carrierId":"INPOST"}]}`))
		case "/shipment-management/shipments/cb92efe4-1b2f-4cac-9e35-da69b82b9482":
			_, _ = w.Write([]byte(`{"id":"cb92efe4-1b2f-4cac-9e35-da69b82b9482","carrier":"ALLEGRO","packages":[{"waybill":"605500867604760112200733"}]}`))
		default:
			t.Errorf("unexpected %s %s", r.Method, r.URL.Path)
			http.NotFound(w, r)
		}
	...
	require.Len(t, found, 1)
	assert.Equal(t, "605500867604760112200733", found[0].Waybill)
	assert.Equal(t, "cb92efe4-1b2f-4cac-9e35-da69b82b9482", found[0].ShipmentID)
	assert.Equal(t, 0, posts)
}
