...
func TestCreateProductListingRequest_Validate(t *testing.T) {
	tests := []struct {
		name    string
		req     CreateProductListingRequest
		wantErr string
	}{
		{
			name: "valid minimal",
		...
		{
			name: "valid with all fields",
			req: func() CreateProductListingRequest {
				extID := "EXT-123"
				url := "https://allegro.pl/offer/123"
				price := 99.99
				stock := 10
				syncMode := "auto"
				return CreateProductListingRequest{
					ProductID:     uuid.New(),
					IntegrationID: uuid.New(),
					ExternalID:    &extID,
					Status:        "active",
		...
		{
			name:    "nil product_id",
			req:     CreateProductListingRequest{ProductID: uuid.Nil, IntegrationID: uuid.New()},
			wantErr: "product_id is required",
		},
		{
			name:    "missing integration_id",
		...
		{
			name:    "nil integration_id",
			req:     CreateProductListingRequest{ProductID: uuid.New(), IntegrationID: uuid.Nil},
			wantErr: "integration_id is required",
		},
	}
...
func TestCreateProductListingRequest_Validate_DefaultStatus(t *testing.T) {
	req := CreateProductListingRequest{ProductID: uuid.New(), IntegrationID: uuid.New()}
	require.NoError(t, req.Validate())
	assert.Equal(t, "pending", req.Status, "status should default to 'pending'")
}

func TestCreateProductListingRequest_Validate_PreservesExplicitStatus(t *testing.T) {
	req := CreateProductListingRequest{ProductID: uuid.New(), IntegrationID: uuid.New(), Status: "active"}
	require.NoError(t, req.Validate())
