...
export default function NewMarketplacePage() {
  ...
  const handleProviderSelect = (providerKey: string) => {
    if (DEDICATED_PROVIDERS.includes(providerKey)) {
      router.push(`/marketplaces/${providerKey}`);
      return;
    }
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" asChild>
            <Link href={step === "configure" ? "#" : "/marketplaces"} onClick={step === "configure" ? (e) => { e.preventDefault(); handleBack(); } : undefined}>
              <ArrowLeft className="h-4 w-4" />
            </Link>
          </Button>
          <div>
            <h1 className="text-2xl font-bold">
              {step === "select" ? "Dodaj marketplace" : `Konfiguracja ${getProviderDisplayName(selectedProvider)}`}
            </h1>
